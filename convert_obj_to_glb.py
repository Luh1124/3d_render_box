...
def convert_files_in_directory(directory, extension):
    ...
    for file in directory.rglob(f'*.{extension}'):
        ...
        subprocess.run(["/baai-cwm-1/baai_cwm_ml/algorithm/hong.li/tools/blender-4.3.2-linux-x64/blender", "-b", "-P", "2gltf2/2gltf2.py", "--", file.resolve(), basename])
...
def convert_file(path):
    ...
    # Call the Blender conversion script for each file
    subprocess.run(["/baai-cwm-1/baai_cwm_ml/algorithm/hong.li/tools/blender-4.3.2-linux-x64/blender", "-b", "-P", "2gltf2/2gltf2.py", "--", path.resolve(), basename])
