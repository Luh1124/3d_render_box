...
if __name__ == "__main__":
    ...
    paser.add_argument('--output_dir', type=str, default="/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/zeroverse/output_all/diligentplane_v1", help='输出目录')
    paser.add_argument('--level', type=int, default=0, help='level')
    ...
    TEXTURE_LIB_BASEPATH = "output_all/matsynth_processed_v2"  
    CSV_PATH = "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/zeroverse/process_tex/matsynth_metal_stats.csv"  
    # GLB_FILES_CSV = "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/utils3dgen/23_unips/glb_animation_check_results_3w9.csv"   # 单一无动画
    ...
    # GLB_FILES = select_glb_files(glbs_info, args.level, num_files=1)  # 随机选择文件

    import glob
    OBJ_FILES = glob.glob("/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/zeroverse/diglientpi/OBJ Files/*.obj")
 
    init_scene()  # 初始化场景
    # 导入并处理 GLB 文件  
    for _ in trange(0, 1000):  
    # for _ in trange(1000, 2000):  

        # 设置输出目录  
        OUTPUT_DIR = os.path.join(args.output_dir, f'level_{args.level}', f"{_:06d}")

        os.makedirs(OUTPUT_DIR, exist_ok=True)   
        
        idx = random.randint(0, len(OBJ_FILES) - 1)
        uid = str(uuid.uuid4())

        blend_save_path = os.path.join(OUTPUT_DIR, f"{uid}.blend")
        path = OBJ_FILES[idx]
        ...
        with open(os.path.join(OUTPUT_DIR, f"{uid}.txt"), 'w') as f:
            f.write(f"uuid: {uid}\n")
            f.write(f"glb_files: {OBJ_FILES}\n")
