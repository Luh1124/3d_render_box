...
class GLBShape(Shape):
    def loadFromGLM(self, glbPath):
        """
        利用 trimesh 加载单个 mesh 的 glb 文件，提取顶点、面、UV、材质及纹理信息。  
        """
        mesh = trimesh.load(glbPath, force='mesh')  # force='mesh' 确保加载为 mesh 类型

        if mesh.is_empty:  
            print(f'Failed to load mesh from {glbPath}')  
            return False  
        if len(mesh.visual.uv) == 0:  
            print(f'No UV coordinates in {glbPath}')  
            return False  

        self.points = np.array(mesh.vertices)  
        self.faces = np.array(mesh.faces) + 1  # OBJ face index 从1开始  
        self.uvs = np.array(mesh.visual.uv) * 1.0  # 确保是浮点  
        # facesUV对应每个face的3个顶点索引的UV，简单映射顶点索引处理  
        self.facesUV = self.faces.copy()  

        # 设置材质名及材质起始索引  
        self.matNames = [os.path.splitext(os.path.basename(glbPath))[0]]  
        self.matStartId = np.array([0], dtype=int) 

        return True

def combineRandomGLBShapes(glbPaths, textureLibPath, outFolder,  
                           min_num=3, max_num=5,  
                           keep_texture_prob=0.1,  
                           use_normal_map=True):  
    """  
    从给定 glb 文件列表中随机选择3-5个组合成一个新物体。  
    纹理按概率决定是否更换或保持。  
    
    参数:  
    - glbPaths: glb文件路径列表  
    - textureLibPath: 纹理库文件夹路径  
    - outFolder: 保存文件夹路径  
    - min_num/max_num: 随机组合数量区间  
    - keep_texture_prob: 保持原纹理概率(0~1)  
    - use_normal_map: 是否在生成mtl时加载法线贴图  
    ...
    num_shapes = random.randint(min_num, max_num)  
    chosen_paths = random.sample(glbPaths, num_shapes)  

    combined_shape = MultiShape(0)  
    combined_shape.__init__(numShape=0)  # 清空初始化  

    for i, path in enumerate(chosen_paths):  
        subShape = GLBShape()  
        if not subShape.loadFromGLB(path):  
            print(f"Skip loading {path}")  
            continue  
        # 可添加纹理替换判断  
        if random.random() > keep_texture_prob:  
            # 替换纹理，设定一个替换纹理名示例  
            subShape.matNames = [f"replaced_texture_{i}"]  
            # 纹理实际替换逻辑自行实现, 此处仅修改材质名方便 genObj 写入  
        combined_shape.addShape(subShape)  

    # 生成obj及mtl  
    combined_shape.matStartId = np.append(combined_shape.matStartId, [len(combined_shape.faces)])  

    # 复写生成obj时的纹理处理，法线贴图加载使用use_normal_map参数控制  
    def genObjWithTextureOptions(self, filePath, bMat=True, bComputeNormal=False, useNormalMap=True):  
        if len(self.faces) == 0:  
            print("no mesh")  
            return False  
        if bComputeNormal:  
        ...
        with open(filePath, "w") as f:  
            ...
            if not bMat:  
                for i, face in enumerate(self.faces):  
                    f.write("f %d/%d %d/%d %d/%d\n" %  
                            (face[0], self.facesUV[i][0],  
                             face[1], self.facesUV[i][1],  
                             face[2], self.facesUV[i][2]))  
            else:  
                for im in range(len(self.matStartId) - 1):  
                    f.write("usemtl %s\n" % self.matNames[im])  
                    ...
                    for i in range(startId, endId):  
                        if not bComputeNormal:  
                            f.write("f %d/%d %d/%d %d/%d\n" %  
                                    (self.faces[i][0], self.facesUV[i][0],  
                                     self.faces[i][1], self.facesUV[i][1],  
                                     self.faces[i][2], self.facesUV[i][2]))  
                        else:  
                            f.write("f %d/%d/%d %d/%d/%d %d/%d/%d\n" %  
                                    (self.faces[i][0], self.facesUV[i][0], self.faces[i][0],  
                                     self.faces[i][1], self.facesUV[i][1], self.faces[i][1],  
        ...
        with open(mtl_path, "w") as f:  
            for im in range(len(self.matStartId) - 1):  
                f.write(f"newmtl {self.matNames[im]}\n")  
                f.write(f"map_Kd {im:02d}_basecolor.png\n")  
                f.write(f"map_Ks {im:02d}_metallic.png\n")  
                f.write(f"map_Pr {im:02d}_roughness.png\n")  
                if useNormalMap:  
    ...
    # 绑定方法到实例  
    import types  
    combined_shape.genObj = types.MethodType(genObjWithTextureOptions, combined_shape)  

    combined_shape.genObj(os.path.join(outFolder, "combined_object.obj"), bMat=True, bComputeNormal=True, useNormalMap=use_normal_map)  

    return combined_shape, chosen_paths  
...
# 示例调用形式  
if __name__ == "__main__":  
    glb_list = ["/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-140/5ccc35586cb843c48b0b4fdbe16c46d9.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-115/631912edaef947f8a345a41f16e15aa7.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-148/06ea30fe0e394fcab3b1793439c473ac.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-004/f9159874daad499492cc0a488e06b489.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-073/7dbbf8b821314967aaba5cde22a02a3d.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-081/32427dedc61b4fd0aacd976246117fe4.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-135/015a03835bef4335a39f2e2022aa2e4c.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-062/4481d9e392ab4175bc5d1508efce6038.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-121/8ae92bdde6ba4ba48cbae3e1f25c1583.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-134/3028f5b45b1b40ba8725a48086973754.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-067/615b8a01e6564ab7b5e81b1ed9fb5aea.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-013/033d8444edb1410a97804ddf03971b0d.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-018/e1fe9cd4ddf648188ed537aae15a5b43.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-159/d7e3e5ba0e6b488e87ef1a7353627863.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-051/053f74ed361a4a10a33a34cedbbe091e.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-054/bc7d825a669d4c30b6a107a9d067e14f.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-049/60d3cb0210bf4c2e9c802cf35122116f.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-111/c305cd94ca214dd5b35c23e94d7c3071.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-101/fcc1aed5629849e6bc6d01bd1ca3c7e6.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-094/90c78e92c4504af5a8a8f6cdec25687b.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-128/407610fac49742a1817506ef81d2b9db.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-096/a87532a3e89947159cc1303008c06eaf.glb", "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/TRELLIS/datasets/ObjaverseXL_sketchfab/raw/glbs/000-108/3066dc0d8f1c48a49c415caf769924a5.glb"]  
    mat_path = get_matsynth_material(out_dir)
    # texture_library_path = "./texture_library"  
    output_folder = "./output_all/output_combined"  

    combined_shape, selected_files = combineRandomGLBShapes(glb_list, texture_library_path, output_folder,  
                                                            min_num=3, max_num=5,  
                                                            keep_texture_prob=0.1,  
                                                            use_normal_map=True)  
    print("Selected glb files combined:", selected_files)  
    print("Combined shape saved to:", output_folder) 