...
def gen_pbr_slat_1_step():
    ...
    for i in range(1,5):
        os.makedirs(out_dir + '/' + f'level_{i}', exist_ok=True)
    ...
    # extract features
    load_queue = Queue(maxsize=10)  # 更大容量减少阻塞  
        ...
        try:  
            for render_path in render_paths:  
                try:  
                    ...
                    if len(positions) == 0:
                        print(f"Warning: indices empty for {render_path}, skipping this sample.")  
                        load_queue.put((render_path, None, None), timeout=120)  # 发送跳过信号
                        continue

                    load_queue.put((render_path, data, positions), timeout=120)  
                except Exception as e:  
                    print(f"Error loading data for {render_path}: {e}")  
        finally:  
            load_queue.put(None)  # 结束信号  

    saver_executor = ThreadPoolExecutor(max_workers=1)  
    ...
    def saver(sha256, pack):  
        try:  
            ...
            print(f"Error saving data for {sha256}: {e}")  

    loader_thread = ThreadPoolExecutor(max_workers=1)  
    loader_thread.submit(data_loader_thread)  

    n_patch = 518 // 14  # 作为全局变量  
    ...
    print(f"Total objects to process: {total}")
    print(f"Output directory: {os.path.join(opt.output_dir, opt.feature_dir)}")

    try:
        while True:
            item = load_queue.get(timeout=120)  # 等待数据加载完成
            if item is None:  # 结束信号
                break

            render_path, data, positions = item

            # 接收到跳过信号  
            if data is None or positions is None:  
                processed_count += 1  
                print(f"Skipped {render_path} due to empty or invalid data. Processed {processed_count}/{total} ({processed_count/total*100:.2f}%)")  
                continue  
            ...
            pack['patchtokens'] = sampled_mean_grid_sample_chunked(patchtokens, uv, num_chunks=12)  

            pack['patchtokens_color'] = sampled_mean_grid_sample_chunked(patchtokens_color, uv, num_chunks=12)  
            pack['patchtokens_brm'] = sampled_mean_grid_sample_chunked(patchtokens_brm, uv, num_chunks=12)  
            ...
            processed_count += 1  
            if processed_count % print_interval == 0 or processed_count == total or processed_count == 1:
                elapsed = time.time() - start_time  
                avg_time = elapsed / processed_count  
                remaining = avg_time * (total - processed_count)  
                remaining_str = str(datetime.timedelta(seconds=int(remaining)))  
                print(f"Processed {processed_count}/{total} objects ({processed_count/total*100:.2f}%), remaining time: {remaining_str}")  
    ...
    loader_thread.shutdown(wait=True)  
