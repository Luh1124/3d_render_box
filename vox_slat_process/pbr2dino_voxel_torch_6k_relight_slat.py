...
def gen_pbr_slat_1_step():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument('--output_dir', type=str, required=False, default='/baai-cwm-vepfs/cwm/hong.li/code/3dgen/data/zeroverse/render_all/3diclight_matsynth_6k_render_slat_0712',
                        help='Directory to save the metadata')
    parser.add_argument('--input_dir', type=str, required=False, default='/baai-cwm-vepfs/cwm/hong.li/code/3dgen/data/zeroverse/render_all/3diclight_matsynth_6k_render_slat_0712/render_relight_150views',
                        help='Directory where the renders are stored')
    parser.add_argument('--feature_dir', type=str, required=False, default='pbr2dino_voxel_latent',
                        help='Directory to save the features')
    parser.add_argument('--model', type=str, default='dinov2_vitl14_reg',
    ...
    # get file list
    if os.path.exists("/baai-cwm-vepfs/cwm/hong.li/code/3dgen/data/zeroverse/metadatas/metadata_3didlight_6k_new_paths.csv"):
        metadata = pd.read_csv("/baai-cwm-vepfs/cwm/hong.li/code/3dgen/data/zeroverse/metadatas/metadata_3didlight_6k_new_paths.csv")
    else:
    ...
    # levels = list(metadata['level'].values)
        ...
        with ThreadPoolExecutor(max_workers=1) as loader_executor, \
            ...
            def loader(render_path):
                try:
                    ...
                    positions = utils3d.io.read_ply(os.path.join("/baai-cwm-vepfs/cwm/hong.li/code/3dgen/data/zeroverse/render_all/3diclight_matsynth_6k_render_slat_0712/voxel", f'{render_path}.ply'))[0]
            ...
            for _ in tqdm(range(len(render_paths)), desc='Processing pbr slat'):
                ...
                pack['patchtokens'] = sampled_mean_grid_sample_chunked(patchtokens, uv, num_chunks=16)  

                pack['patchtokens_color'] = sampled_mean_grid_sample_chunked(patchtokens_color, uv, num_chunks=16)  
                pack['patchtokens_brm'] = sampled_mean_grid_sample_chunked(patchtokens_brm, uv, num_chunks=16)  
