...
def gen_pbr_slat_1_step():
    ...
    # parser.add_argument('--feature_dir', type=str, required=False, default='pbr_surface_slats',
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as loader_executor, \
            ...
            def saver(render_path, pack, all_pts, all_features):
                # 将坐标映射到 [0, 64) 整数范围
                ...
                pack['positions'] = voxel_centers.cpu().numpy()
                pack['pbr_features'] = mean_features.cpu().numpy()

                save_path = os.path.join(opt.output_dir, opt.feature_dir, render_path+'.npz')
            ...
            for _ in tqdm(range(len(render_paths)), desc='Processing pbr slat'):
                ...
                for i in range(0, n_views, opt.batch_size):
                    ...
                    # 深度图转换为点云
                    pts = depth_to_points(batch_depth, batch_intrinsics, batch_extrinsics)
                    ...
                    base_color_valid = batch_color.permute(0,2,3,1)[batch_mask > 0]
                    ...
                    features_valid_lst.append(torch.cat([roughness_valid, metallic_valid, normal_valid, base_color_valid], dim=1))

                all_pts = torch.cat(pts_valid_lst, dim=0)  # (N_total, 3)
                all_features = torch.cat(features_valid_lst, dim=0)  # (N_total, 9)

                saver_executor.submit(saver, render_path, pack, all_pts, all_features)
