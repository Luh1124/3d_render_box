...
def main(args):
    ...
    hdri_file_dir = "/baai-cwm-backup/cwm/hong.li/data/high_4k_hdri/4k_exr"
    ...
    for i, view in enumerate(views):
        ...
        # cam.matrix_world = Matrix(view["c2w"])

        # randomize_lighting()
        # add_light_env(env=(0.35, 0.35, 0.35), strength = 1.0)
        # point_lighting["default_light"].location64 position
        hdri_file_path = os.path.join(hdri_file_dir, view["hdri_file_path"])
        # rotation_euler = (0, 0, np.random.uniform(0, 2*np.pi))
        rotation_euler = (0, 0, 0)
        set_hdri(path_to_hdr_file=hdri_file_path, strength=1.0, rotation_euler=rotation_euler)

        # 环境光照
        render_current_scene(file_postfix="random_area", i=view["index"], j=0, args=args, save_light=True, outputs=outputs)
        # remove the cache
        ...
        metadata = {
            ...
            "hdri_file_path": os.path.basename(hdri_file_path),
