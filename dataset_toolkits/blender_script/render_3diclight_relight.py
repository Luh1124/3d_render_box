...
def randomize_lighting() -> None:
    ...
    light2.energy = np.random.uniform(300, 500)
    bpy.data.objects["Area-random"].location[0] = np.random.uniform(-1., 1.)
    bpy.data.objects["Area-random"].location[1] = np.random.uniform(-1., 1.)
    bpy.data.objects["Area-random"].location[2] = np.random.uniform(0.5, 1.5)
...
def main(args):
    ...
    if args.object.endswith(".blend"):
        ...
        base_path = "/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/zeroverse/output_all/"  
    ...
    hdr_file_paths = sorted(glob.glob("/baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/utils3dgen/12_HDRI_data/high_4k_hdri/4k_exr/*.exr"))
    ...
    for i, view in enumerate(views):
        ...
        render_current_scene(file_postfix="random_area", i=i, j=0, args=args, save_light=True, outputs=outputs)
        # Save camera parameters
        metadata = {
            "file_path": os.path.basename(bpy.context.scene.render.filepath),
            "camera_angle_x": 40 / 180 * np.pi,
            "transform_matrix": get_transform_matrix(cam),
            # "hdri_file_path": args.hdri_file_path,
            # "rotation_euler": rotation_eulers[j],
            # "point_lighting_energy": point_lighting_energy,
            # "point_lighting_location": point_lighting_location,
            # "area_lighting_energy": area_lighting_energy,
            # "area_lighting_location": area_lighting_location,
            # "area_lighting_scale": area_lighting_scale,
        }
    ...
    for i, view in enumerate(views):
        ...
        # point_lighting_offest = (np.random.rand(), np.random.rand())
        # area_lighting_offest = (np.random.rand(), np.random.rand())
        for j in range(args.num_lights):
            hdri_file_path = hdr_file_paths[np.random.randint(0, len(hdr_file_paths))]
            point_lighting_yaw, point_lighting_pitch = hemisphere_uniform_sequence()
            rotation_euler = (0, 0, np.random.uniform(0, 2*np.pi))


            # point_lighting["default_light"].location64 position
            hdri_node = set_hdri(path_to_hdr_file=hdri_file_path, strength=1.0, rotation_euler=rotation_euler)

            # 环境光照
            render_current_scene(file_postfix="env", i=i, j=j, args=args, save_light=False, outputs=outputs)
            # remove the cache
            bpy.data.images.remove(bpy.data.images[os.path.basename(hdri_file_path)])  # 释放 HDR 贴图
