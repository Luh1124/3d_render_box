...
"""
512 分辨率 3diclight 可选光照数量 dense views relight images
透视相机
资产渲染 depth、RGB、PBR、normal
"""
...
def _render(file_path, sha256, level, output_dir, num_views=12, low_normal=True, out_basename="render"):
    ...
    args = [
        BLENDER_PATH, '-b', '-P', os.path.join(os.path.dirname(__file__), 'blender_script', 'render_3diclight_relight_slat.py'),
        '--',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default="render_all/3diclight_matsynth_6k_render_slat_0712",
                        help='Directory to save the metadata')
    parser.add_argument('--metadata_dir', type=str, default="metadatas",
                        help='Directory to save the metadata')
    parser.add_argument('--num_views', type=int, default=150,
                        help='Number of views to render')
    parser.add_argument('--out_basename', type=str, default="render_relight_150views", help='The basename of the output directory')
    ...
    # get file list
    if not os.path.exists(os.path.join(opt.metadata_dir, 'metadata_3didlight_6k_new_paths.csv')):
        raise ValueError('metadata_3didlight_6k_new_paths.csv not found')
    metadata = pd.read_csv(os.path.join(opt.metadata_dir, 'metadata_3didlight_6k_new_paths.csv'))
