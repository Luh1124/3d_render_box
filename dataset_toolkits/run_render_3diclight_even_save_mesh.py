...
def _render(file_path, sha256, level, output_dir, num_views=1, num_lights=10, low_normal=True, out_basename="render"):
    ...
    args = [
        BLENDER_PATH, '-b', '-P', os.path.join(os.path.dirname(__file__), 'blender_script', 'render_3diclight_even_mesh.py'),
        '--',
...
def check_processed(sha256, level, output_dir, out_basename):  
    ...
    # return sha256 if os.path.exists(path) and os.path.exists(low_path) else None 
    mesh_path = os.path.join(output_dir, out_basename, f'level_{level}', sha256, 'mesh.ply')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_basename', type=str, default="even_render_6k_mesh", help='The basename of the output directory')
