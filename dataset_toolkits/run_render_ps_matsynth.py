...
if __name__ == '__main__':
    ...
    # get file list
    if not os.path.exists(os.path.join(opt.metadata_dir, 'metadata_PS_v2_6w.csv')):
        raise ValueError('metadata_PS_v2_6w.csv not found')
    metadata = pd.read_csv(os.path.join(opt.metadata_dir, 'metadata_PS_v2_6w.csv'))

    # 随机选取一部分数据
    # metadata = metadata.sample(frac=0.2, random_state=42)
