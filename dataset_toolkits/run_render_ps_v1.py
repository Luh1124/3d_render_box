...
def _render(file_path, sha256, level, output_dir, num_views=1, num_lights=10, low_normal=True, out_basename="render"):
    ...
    args = [
        BLENDER_PATH, '-b', '-P', os.path.join(os.path.dirname(__file__), 'blender_script', 'render_ps_v1.py'),
        '--',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_basename', type=str, default="PS_v2_render", help='The basename of the output directory')
    ...
    # get file list
    if not os.path.exists(os.path.join(opt.metadata_dir, 'metadata_ps_v2.csv')):
        raise ValueError('metadata_ps_v2.csv not found')
    metadata = pd.read_csv(os.path.join(opt.metadata_dir, 'metadata_ps_v2.csv'))
