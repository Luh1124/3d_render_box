...
def _voxelize(file, sha256, level, output_dir, out_basename):
    mesh = o3d.io.read_triangle_mesh(os.path.join(output_dir, 'even', f'{level}', sha256, 'mesh.ply'))
    ...
    utils3d.io.write_ply(os.path.join(output_dir, out_basename, f'{level}', f'{sha256}.ply'), vertices)
    return {'sha256': sha256, 'voxelized': True, 'num_voxels': len(vertices)}


def check_processed(sha256, level, output_dir, out_basename):  
    mesh_path = os.path.join(output_dir, out_basename, f'{level}', f'{sha256}.ply')
...
if __name__ == '__main__':
    ...
    for level in range(1, 4):
        os.makedirs(os.path.join(output_dir, f'Level{level}'), exist_ok=True)
