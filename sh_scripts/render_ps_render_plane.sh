...
# Default parameters  
DEFAULT_OUTPUT_DIR="render_all"  
DEFAULT_SUBSET="unips"  
DEFAULT_OUTBASE="PS_plane_2K_render"  # Default output base name
RANK=0
WORLD_SIZE=6
MAX_WORKERS=12
...
echo "python dataset_toolkits/run_render_ps_plane.py $SUBSET --output_dir $OUTPUT_DIR --out_basename $OUTBASE --rank $RANK --world_size $WORLD_SIZE --max_workers $MAX_WORKERS"
...
python dataset_toolkits/run_render_ps_plane.py $SUBSET --output_dir $OUTPUT_DIR --out_basename $OUTBASE --rank $RANK --world_size $WORLD_SIZE --max_workers $MAX_WORKERS
