...
mkdir -p logs/6k/  
LOGFILE="logs/6k/pbr2dino_voxel_torch_rank${RANK}_worldsize${WORLD_SIZE}.log"

CUDA_VISIBLE_DEVICES=0 python pbr2dino_voxel_torch.py \
 --rank $RANK \
 --world_size $WORLD_SIZE  > "$LOGFILE" 2>&1 

wait  

echo "All extract dino slat tasks for rank '$RANK' and world size '$WORLD_SIZE' completed, output saved in '$LOGFILE'."