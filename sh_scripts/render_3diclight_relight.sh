...
cd /baai-cwm-1/baai_cwm_ml/cwm/hong.li/code/3dgen/data/zeroverse
...
# Default parameters  
DEFAULT_OUTPUT_DIR="3diclight_matsynth"  
DEFAULT_SUBSET="3diclight"  
DEFAULT_OUTBASE="relight_render_6k"  # Default output base name
RANK=0
WORLD_SIZE=6
MAX_WORKERS=12
...
echo "python dataset_toolkits/run_render_3diclight_relight.py $SUBSET --output_dir $OUTPUT_DIR --out_basename $OUTBASE --rank $RANK --world_size $WORLD_SIZE --max_workers $MAX_WORKERS"
...
python dataset_toolkits/run_render_3diclight_relight.py $SUBSET --output_dir $OUTPUT_DIR --out_basename $OUTBASE --rank $RANK --world_size $WORLD_SIZE --max_workers $MAX_WORKERS
